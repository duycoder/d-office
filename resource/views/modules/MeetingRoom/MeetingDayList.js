...
class MeetingDayList extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      data: [],
      typeLich: LICHTRUC_CONSTANT.CHUYEN_MON,
  ...
  async fetchData() {
    const url = `${API_URL}/api/Lichtruc/MeetingDayList/${this.state.pageSize}/${this.state.pageIndex}/${this.state.userId}/${this.state.typeLich}?query=${this.state.filterValue}`;
  ...
  async loadItems(day) {
    const startDate = convertDateToString(day.timestamp - 15 * 86400000),
      endDate = convertDateToString(day.timestamp + 15 * 86400000);
